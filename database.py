"""Módulo de Persistência — SQLAlchemy Postgres para OrgAudi."""
...
class NotaModel(Base):
    ...
    # Novos campos para Auditoria 2026
    laudo_ia = Column(String) # Texto completo da análise IA para esta nota/lote
    data_auditoria = Column(DateTime, server_default=func.now())
...
def criar_tabelas():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            try:
                conn.execute(text("ALTER TABLE notas ADD COLUMN IF NOT EXISTS laudo_ia TEXT"))
                conn.execute(text("ALTER TABLE notas ADD COLUMN IF NOT EXISTS data_auditoria TIMESTAMP DEFAULT now()"))
                conn.commit()
            except Exception as e:
                logger.warning(f"Aviso na migração de colunas (provavelmente já existem): {e}")
...
def salvar_notas_bd(notas: list[NFA], laudo_texto: str = None) -> tuple[int, int]:
        ...
        for nfa in notas:
            ...
            try:
                ...
                nova_nota = NotaModel(
                    ...
                    laudo_ia=laudo_texto, # Salva o laudo consolidado no registro
...
def listar_ultimas_auditorias(limit=100):
    """Retorna as últimas notas auditadas com seus laudos."""
    with SessionLocal() as db:
        return (
            db.query(NotaModel)
            .options(joinedload(NotaModel.destinatario))
