...
def auditoria_consolidada(notas: list[NFA], callback=None, nome_contribuinte: str = "") -> str:
    """Orquestra a Auditoria Completa: Ípsilon -> Sigma -> Gama -> Auditor Diretor."""
    if callback:
        msg = "\n\n[SQUAD] INICIANDO AUDITORIA TRIPARTITE UNIFICADA (CLAUDE)...\n"
        if nome_contribuinte: msg = msg.replace("...", f" PARA {nome_contribuinte}...")
        callback(msg)
    
    # Agora usamos o pipeline consolidado no ai_client que já faz os 3 estágios
    return analisar_pipeline(notas, callback=callback, nome_contribuinte=nome_contribuinte)
