...
def main() -> None:
    ...
    try:
        notas, nome_contribuinte, cpf_contribuinte = extrair_notas(str(pdf_path))
    except Exception as e:
        print(f'Erro ao extrair notas: {e}', file=sys.stderr)
        sys.exit(1)

    print(f'  > {len(notas)} notas encontradas para {nome_contribuinte or "Contribuinte Desconhecido"}')
    ...
    exportar_excel(notas, saida, nome_contribuinte=nome_contribuinte)
    print(f'Salvo: {saida}')
